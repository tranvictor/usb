...
func enumerateHid(vendorID uint16, productID uint16) ([]DeviceInfo, error) {
	...
	head := C.v_hid_enumerate(C.ushort(vendorID), C.ushort(productID))
	if head == nil {
		return nil, nil
	}
	defer C.v_hid_free_enumeration(head)
...
func openHid(info DeviceInfo) (*hidDevice, error) {
	...
	device := C.v_v_hid_open_path(path)
	if device == nil {
...
func (dev *hidDevice) Write(b []byte) (int, error) {
	...
	written := int(C.v_hid_write(device, (*C.uchar)(&report[0]), C.size_t(len(report))))
	if written == -1 {
...
func (dev *hidDevice) Read(b []byte) (int, error) {
	...
	read := int(C.v_hid_read(device, (*C.uchar)(&b[0]), C.size_t(len(b))))
	if read == -1 {
