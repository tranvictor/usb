...
const size_t v_SIZEOF_WCHAR_T = sizeof(wchar_t);

void v_gowchar_set (wchar_t *arr, int pos, wchar_t val)
{
	arr[pos] = val;
}

wchar_t v_gowchar_get (wchar_t *arr, int pos)
{
	return arr[pos];
}
*/
...
var sizeofWcharT C.size_t = C.size_t(C.v_SIZEOF_WCHAR_T)
...
func stringToWchar2(s string) (*C.wchar_t, C.size_t) {
	...
	for len(s) > 0 {
		...
		if r1, r2 := utf16.EncodeRune(r); r1 != '\uFFFD' {
			C.v_gowchar_set((*C.wchar_t)(res), C.int(i), C.wchar_t(r1))
			i++
			C.v_gowchar_set((*C.wchar_t)(res), C.int(i), C.wchar_t(r2))
			i++
		} else {
			C.v_gowchar_set((*C.wchar_t)(res), C.int(i), C.wchar_t(r))
			i++
		}
		s = s[size:]
	}
	C.v_gowchar_set((*C.wchar_t)(res), C.int(slen-1), C.wchar_t(0)) // \0
