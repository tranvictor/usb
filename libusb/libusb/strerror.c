...
/** \ingroup libusb_misc
 * How to add a new \ref v_libusb_strerror() translation:
 * <ol>
 ...
 *
 * If v_libusb_setlocale() is not called, all messages will be in English.
 *
 * The following functions return translatable strings: v_libusb_strerror().
 * Note that the libusb log messages controlled through v_libusb_set_debug()
 * are not translated, they are always in English.
 *
 * For POSIX UTF-8 environments if you want libusb to follow the standard
 * locale settings, call v_libusb_setlocale(setlocale(LC_MESSAGES, NULL)),
...
int API_EXPORTED v_libusb_setlocale(const char *locale)
{
	size_t i;
 ...
 * the language set by v_libusb_setlocale().
 *
 ...
 */
DEFAULT_VISIBILITY const char* LIBUSB_CALL v_libusb_strerror(enum libusb_error errcode)
{
