...
int v_usbi_parse_descriptor(const unsigned char *source, const char *descriptor,
	...
	int size, int host_endian)
{
	...
	v_usbi_parse_descriptor(buffer, "bb", &header, 0);
	...
	if (header.bLength >= ENDPOINT_AUDIO_DESC_LENGTH)
		v_usbi_parse_descriptor(buffer, "bbbbwbbb", endpoint, host_endian);
	else if (header.bLength >= ENDPOINT_DESC_LENGTH)
		v_usbi_parse_descriptor(buffer, "bbbbwb", endpoint, host_endian);
	else {
	...
	int host_endian)
{
	...
	while (size >= INTERFACE_DESC_LENGTH) {
		...
		v_usbi_parse_descriptor(buffer, "bbbbbbbbb", ifp, 0);
	...
	int size, int host_endian)
{
	...
	v_usbi_parse_descriptor(buffer, "bbwbbbbb", config, host_endian);
...
int v_usbi_device_cache_descriptor(libusb_device *dev)
{
	int r, host_endian = 0;

	r = v_usbi_backend.get_device_descriptor(dev, (unsigned char *) &dev->device_descriptor,
						&host_endian);
	if (r < 0)
		return r;
 ...
 */
int API_EXPORTED v_v_libusb_get_device_descriptor(libusb_device *dev,
 ...
 * valid if 0 was returned. Must be freed with v_libusb_free_config_descriptor()
 * after use.
 * \returns 0 on success
 * \returns LIBUSB_ERROR_NOT_FOUND if the device is in unconfigured state
 * \returns another LIBUSB_ERROR code on error
 * \see v_libusb_get_config_descriptor
 */
int API_EXPORTED v_libusb_get_active_config_descriptor(libusb_device *dev,
	struct libusb_config_descriptor **config)
{
	...
	r = v_usbi_backend.get_active_config_descriptor(dev, tmp,
	...
	v_usbi_parse_descriptor(tmp, "bbw", &_config, host_endian);
	buf = malloc(_config.wTotalLength);
	if (!buf)
		return LIBUSB_ERROR_NO_MEM;

	r = v_usbi_backend.get_active_config_descriptor(dev, buf,
 ...
 * \returns another LIBUSB_ERROR code on error
 * \see v_libusb_get_active_config_descriptor()
 * \see v_v_libusb_get_config_descriptor_by_value()
 */
int API_EXPORTED v_libusb_get_config_descriptor(libusb_device *dev,
	uint8_t config_index, struct libusb_config_descriptor **config)
{
	...
	r = v_usbi_backend.get_config_descriptor(dev, config_index, tmp,
		LIBUSB_DT_CONFIG_SIZE, &host_endian);
	if (r < 0)
		return r;
	if (r < LIBUSB_DT_CONFIG_SIZE) {
	...
	r = v_usbi_backend.get_config_descriptor(dev, config_index, buf,
		_config.wTotalLength, &host_endian);
	if (r >= 0)
		r = raw_desc_to_config(dev->ctx, buf, r, host_endian, config);
 ...
 */
int v_usbi_get_config_index_by_value(struct libusb_device *dev,
	uint8_t bConfigurationValue, int *idx)
{
	...
	for (i = 0; i < dev->num_configurations; i++) {
		unsigned char tmp[6];
		int host_endian;
		int r = v_usbi_backend.get_config_descriptor(dev, i, tmp, sizeof(tmp),
			&host_endian);
		if (r < 0) {
			*idx = -1;
			return r;
 ...
 * \see v_libusb_get_config_descriptor()
 */
int API_EXPORTED v_v_libusb_get_config_descriptor_by_value(libusb_device *dev,
	uint8_t bConfigurationValue, struct libusb_config_descriptor **config)
{
	int r, idx, host_endian;
	unsigned char *buf = NULL;

	if (v_usbi_backend.get_config_descriptor_by_value) {
		r = v_usbi_backend.get_config_descriptor_by_value(dev,
	...
	r = v_usbi_get_config_index_by_value(dev, bConfigurationValue, &idx);
	if (r < 0)
		return r;
	else if (idx == -1)
		return LIBUSB_ERROR_NOT_FOUND;
	else
		return v_libusb_get_config_descriptor(dev, (uint8_t) idx, config);
}

/** \ingroup libusb_desc
 * Free a configuration descriptor obtained from
 * v_libusb_get_active_config_descriptor() or v_libusb_get_config_descriptor().
 * It is safe to call this function with a NULL config parameter, in which
 * case the function simply returns.
 *
 * \param config the configuration descriptor to free
 */
void API_EXPORTED v_libusb_free_config_descriptor(
	struct libusb_config_descriptor *config)
{
	if (!config)
		return;
 ...
 * v_libusb_free_ss_endpoint_companion_descriptor() after use.
 * \returns 0 on success
 * \returns LIBUSB_ERROR_NOT_FOUND if the configuration does not exist
 * \returns another LIBUSB_ERROR code on error
 */
int API_EXPORTED v_libusb_get_ss_endpoint_companion_descriptor(
	...
	*ep_comp = NULL;
	...
	while (size >= DESC_HEADER_LENGTH) {
		...
		*ep_comp = malloc(sizeof(**ep_comp));
		if (*ep_comp == NULL)
			return LIBUSB_ERROR_NO_MEM;
		v_usbi_parse_descriptor(buffer, "bbbbw", *ep_comp, 0);
		return LIBUSB_SUCCESS;
	}
	return LIBUSB_ERROR_NOT_FOUND;
}
 ...
 */
void API_EXPORTED v_libusb_free_ss_endpoint_companion_descriptor(
	struct libusb_ss_endpoint_companion_descriptor *ep_comp)
{
	free(ep_comp);
}
	...
	unsigned char *buffer, int size, int host_endian)
{
	...
	v_usbi_parse_descriptor(buffer, "bbwb", &bos_header, host_endian);
	...
	v_usbi_parse_descriptor(buffer, "bbwb", _bos, host_endian);
	...
	for (i = 0; i < bos_header.bNumDeviceCaps; i++) {
		...
		v_usbi_parse_descriptor(buffer, "bbb", &dev_cap, host_endian);
		...
		if (dev_cap.bLength < LIBUSB_DT_DEVICE_CAPABILITY_SIZE) {
			usbi_err(ctx, "invalid dev-cap bLength (%d)",
				 dev_cap.bLength);
			v_libusb_free_bos_descriptor(_bos);
			return LIBUSB_ERROR_IO;
		}
 ...
 * Must be freed with \ref v_libusb_free_bos_descriptor() after use.
 * \returns 0 on success
 * \returns LIBUSB_ERROR_NOT_FOUND if the device doesn't have a BOS descriptor
 * \returns another LIBUSB_ERROR code on error
 */
int API_EXPORTED v_libusb_get_bos_descriptor(libusb_device_handle *dev_handle,
	struct libusb_bos_descriptor **bos)
{
	...
	v_usbi_parse_descriptor(bos_header, "bbwb", &_bos, host_endian);
...
/** \ingroup libusb_desc
 * Free a BOS descriptor obtained from v_libusb_get_bos_descriptor().
 * It is safe to call this function with a NULL bos parameter, in which
 * case the function simply returns.
 *
 * \param bos the BOS descriptor to free
 */
void API_EXPORTED v_libusb_free_bos_descriptor(struct libusb_bos_descriptor *bos)
{
	int i;
 ...
 * descriptor. Only valid if 0 was returned. Must be freed with
 * v_libusb_free_usb_2_0_extension_descriptor() after use.
 * \returns 0 on success
 * \returns a LIBUSB_ERROR code on error
 */
int API_EXPORTED v_libusb_get_usb_2_0_extension_descriptor(
	struct libusb_context *ctx,
	struct libusb_bos_dev_capability_descriptor *dev_cap,
	struct libusb_usb_2_0_extension_descriptor **usb_2_0_extension)
{
	...
	v_usbi_parse_descriptor((unsigned char *)dev_cap, "bbbd",
 ...
 */
void API_EXPORTED v_libusb_free_usb_2_0_extension_descriptor(
	struct libusb_usb_2_0_extension_descriptor *usb_2_0_extension)
{
 ...
 * v_libusb_free_ss_usb_device_capability_descriptor() after use.
 * \returns 0 on success
 * \returns a LIBUSB_ERROR code on error
 */
int API_EXPORTED v_libusb_get_ss_usb_device_capability_descriptor(
	struct libusb_context *ctx,
	struct libusb_bos_dev_capability_descriptor *dev_cap,
	struct libusb_ss_usb_device_capability_descriptor **ss_usb_device_cap)
{
	...
	v_usbi_parse_descriptor((unsigned char *)dev_cap, "bbbbwbbw",
 ...
 */
void API_EXPORTED v_libusb_free_ss_usb_device_capability_descriptor(
	struct libusb_ss_usb_device_capability_descriptor *ss_usb_device_cap)
{
 ...
 * v_libusb_free_container_id_descriptor() after use.
 * \returns 0 on success
 * \returns a LIBUSB_ERROR code on error
 */
int API_EXPORTED v_libusb_get_container_id_descriptor(struct libusb_context *ctx,
	struct libusb_bos_dev_capability_descriptor *dev_cap,
	struct libusb_container_id_descriptor **container_id)
{
	...
	v_usbi_parse_descriptor((unsigned char *)dev_cap, "bbbbu",
 ...
 */
void API_EXPORTED v_libusb_free_container_id_descriptor(
	struct libusb_container_id_descriptor *container_id)
{
	free(container_id);
}
 ...
 */
int API_EXPORTED v_libusb_get_string_descriptor_ascii(libusb_device_handle *dev_handle,
