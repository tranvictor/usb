	...
	DEVINST devinst, PDEVINST _parent_devinst)
{
	...
	while (dev == NULL) {
		if (CM_Get_Parent(&parent_devinst, devinst, 0) != CR_SUCCESS)
			break;
		devinst = parent_devinst;
		dev = v_usbi_get_device_by_session_id(ctx, (unsigned long)devinst);
	}
...
static int windows_assign_endpoints(struct libusb_device_handle *dev_handle, int iface, int altsetting)
{
	...
	r = v_libusb_get_active_config_descriptor(dev_handle->dev, &conf_desc);
	if (r != LIBUSB_SUCCESS) {
	...
	if (current_interface < 0) { // No serviceable interface was found
		for (current_interface = 0; current_interface < USB_MAXINTERFACES; current_interface++) {
			...
			if ((priv->usb_interface[current_interface].apib->id == api_type)
					&& (v_libusb_claim_interface(transfer->dev_handle, current_interface) == LIBUSB_SUCCESS)) {
...
static void auto_release(struct usbi_transfer *itransfer)
{
	...
	if (handle_priv->autoclaim_count[transfer_priv->interface_number] > 0) {
		...
		if (handle_priv->autoclaim_count[transfer_priv->interface_number] == 0) {
			r = v_libusb_release_interface(dev_handle, transfer_priv->interface_number);
			if (r == LIBUSB_SUCCESS)
				usbi_dbg("auto-released interface %d", transfer_priv->interface_number);
			else
				usbi_dbg("failed to auto-release interface %d (%s)",
					transfer_priv->interface_number, v_libusb_error_name((enum libusb_error)r));
		}
	}
	usbi_mutex_unlock(&autoclaim_lock);
}
	...
	uint8_t bus_number, DEVINST devinst)
{
	...
	dev = v_usbi_get_device_by_session_id(ctx, session_id);
	if (dev == NULL) {
...
//#define ENUM_DEBUG
		...
		for (i = 0; ; i++) {
			...
			if (pass >= GEN_PASS) {
				...
				if (parent_dev == NULL) {
					...
					dev = v_usbi_get_device_by_session_id(ctx, (unsigned long)dev_info_data.DevInst);
					if (dev != NULL) {
						priv = _device_priv(dev);
						if (priv->root_hub)
							goto track_unref;
				...
				if ((pass == GEN_PASS) && (parent_priv->apib->id != USB_API_HUB)) {
					v_libusb_unref_device(parent_dev);
					continue;
				}
			}
			...
			if ((pass <= GEN_PASS) && (pass != HCD_PASS)) {	// For subsequent passes, we'll lookup the parent
				...
				alloc_device:
					...
					dev = v_usbi_alloc_device(ctx, session_id);
					if (dev == NULL)
						LOOP_BREAK(LIBUSB_ERROR_NO_MEM);
			...
			case GEN_PASS:
				...
				if (r == LIBUSB_SUCCESS) {
					...
					discdevs = v_discovered_devs_append(*_discdevs, dev);
					if (!discdevs)
						LOOP_BREAK(LIBUSB_ERROR_NO_MEM);
	...
	for (i = 0; i < unref_cur; i++)
		v_libusb_unref_device(unref_list[i]);
	free(unref_list);
...
static int winusb_set_configuration(struct libusb_device_handle *dev_handle, int config)
{
	...
	r = v_libusb_control_transfer(dev_handle, LIBUSB_ENDPOINT_OUT |
	...
	int (*transfer_fn)(int, struct usbi_transfer *))
{
	...
	r = v_usbi_add_pollfd(ctx, wfd.fd, events);
	if (r) {
		usbi_close(wfd.fd);
		return r;
	}
	...
	if ((r != LIBUSB_SUCCESS) && (r != LIBUSB_ERROR_OVERFLOW)) {
		v_usbi_remove_pollfd(ctx, wfd.fd);
		usbi_close(wfd.fd);
		transfer_priv->pollable_fd = INVALID_WINFD;
	}
...
/*
	if (available[SUB_API_MAX]) // HID driver
		r = v_hid_open(SUB_API_NOTSET, dev_handle);
*/
	return r;
}
...
/*
	if (available[SUB_API_MAX]) // HID driver
		v_hid_close(SUB_API_NOTSET, dev_handle);
*/
}
...
static int composite_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer)
{
	...
	case LIBUSB_RECIPIENT_ENDPOINT:
		r = v_libusb_get_active_config_descriptor(transfer->dev_handle->dev, &conf_desc);
		if (r == LIBUSB_SUCCESS) {
