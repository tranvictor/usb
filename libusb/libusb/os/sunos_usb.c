...
sunos_add_devices(di_devlink_t link, void *arg)
{
	...
	for (dn = di_child_node(myself); dn != DI_NODE_NIL; dn = di_sibling_node(dn)) {
		...
		dev = v_usbi_get_device_by_session_id(nargs->ctx, sid);
		if (dev == NULL) {
			dev = v_usbi_alloc_device(nargs->ctx, sid);
			if (dev == NULL) {
			...
			if (v_usbi_sanitize_device(dev) < 0) {
				v_libusb_unref_device(dev);
		...
		if (v_discovered_devs_append(*(nargs->discdevs), dev) == NULL) {
    ...
    uint8_t endpoint, uint8_t *interface)
{
	...
	r = v_libusb_get_active_config_descriptor(hdev->dev, &config);
	if (r < 0) {
...
sunos_async_callback(union sigval arg)
{
	...
	/* async notification */
	v_usbi_signal_transfer_completion(LIBUSB_TRANSFER_TO_USBI_TRANSFER(xfer));
}
...
solaris_submit_ctrl_on_default(struct libusb_transfer *transfer)
{
	...
	ret = v_usbi_handle_transfer_completion(LIBUSB_TRANSFER_TO_USBI_TRANSFER(transfer),
	    transfer->status);
...
sunos_clear_halt(struct libusb_device_handle *handle, uint8_t endpoint)
{
	...
	ret = v_libusb_control_transfer(handle, LIBUSB_ENDPOINT_OUT |
...
sunos_cancel_transfer(struct usbi_transfer *itransfer)
{
	...
	/*
	 * we don't need to call v_usbi_handle_transfer_cancellation(),
...
{
	return v_usbi_handle_transfer_completion(itransfer, LIBUSB_TRANSFER_COMPLETED);
}
