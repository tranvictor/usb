 ...
 * simply assume /dev/bus/usb rather then making v_libusb_init fail. */
#if defined(USE_UDEV)
	if (ret == NULL)
		ret = "/dev/bus/usb";
#endif
	...
	uint8_t descriptor_type, unsigned char *buffer, int size)
{
	...
	for (i = 0; size >= 0; i += header.bLength, size -= header.bLength) {
		...
		v_usbi_parse_descriptor(buffer + i, "bb", &header, 0);
	...
	unsigned char *buffer, int size)
{
	...
	v_usbi_parse_descriptor(buffer, "bbwbbbbb", &config, 0);
	...
	list_for_each_entry(it, &ctx->usb_devs, list, struct libusb_device) {
		...
		if (priv->sysfs_dir) {
			if (0 == strcmp (priv->sysfs_dir, parent_sysfs_dir)) {
				dev->parent_dev = v_libusb_ref_device(it);
				break;
			}
		}
	}
...
static int op_open(struct libusb_device_handle *handle)
{
	...
	r = v_usbi_add_pollfd(HANDLE_CTX(handle), hpriv->fd, POLLOUT);
	if (r < 0)
		close(hpriv->fd);
...
static void op_close(struct libusb_device_handle *dev_handle)
{
	...
	if (!hpriv->fd_removed)
		v_usbi_remove_pollfd(HANDLE_CTX(dev_handle), hpriv->fd);
	close(hpriv->fd);
}
...
completed:
	...
	return CANCELLED == tpriv->reap_action ?
		v_usbi_handle_transfer_cancellation(itransfer) :
		v_usbi_handle_transfer_completion(itransfer, tpriv->reap_status);
}
	...
	struct usbfs_urb *urb)
{
	...
	if (tpriv->reap_action != NORMAL) { /* cancelled or submit_fail */
		...
		if (tpriv->num_retired == num_urbs) {
			...
			if (tpriv->reap_action == CANCELLED) {
				usbi_mutex_unlock(&itransfer->lock);
				return v_usbi_handle_transfer_cancellation(itransfer);
			} else {
				usbi_mutex_unlock(&itransfer->lock);
				return v_usbi_handle_transfer_completion(itransfer,
	...
	if (urb_idx == num_urbs) {
		...
		free_iso_urbs(tpriv);
		usbi_mutex_unlock(&itransfer->lock);
		return v_usbi_handle_transfer_completion(itransfer, status);
	}
	...
	struct pollfd *fds, POLL_NFDS_TYPE nfds, int num_ready)
{
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		if (pollfd->revents & POLLERR) {
			...
			v_usbi_remove_pollfd(HANDLE_CTX(handle), hpriv->fd);
			hpriv->fd_removed = 1;
			...
			v_usbi_handle_disconnect(handle);
			continue;
		}
