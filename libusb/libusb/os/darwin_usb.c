...
_Atomic int32_t v_initCount = ATOMIC_VAR_INIT(0);
#else
...
static volatile int32_t v_initCount = 0;
...
static void darwin_devices_attached (void *ptr, io_iterator_t add_devices) {
  ...
  usbi_mutex_lock(&v_active_contexts_lock);
  ...
  usbi_mutex_unlock(&v_active_contexts_lock);
}
...
static void darwin_devices_detached (void *ptr, io_iterator_t rem_devices) {
  ...
  while ((device = IOIteratorNext (rem_devices)) != 0) {
    ...
    list_for_each_entry(ctx, &v_active_contexts_list, list, struct libusb_context) {
      ...
      dev = v_usbi_get_device_by_session_id(ctx, (unsigned long) session);
      if (dev) {
...
static int darwin_init(struct libusb_context *ctx) {
  ...
  if (libusb_darwin_atomic_fetch_add (&v_initCount, 1) == 0) {
#if !OSX_USE_CLOCK_GETTIME
...
static void darwin_exit (struct libusb_context *ctx) {
  UNUSED(ctx);
  if (libusb_darwin_atomic_fetch_add (&v_initCount, -1) == 1) {
#if !OSX_USE_CLOCK_GETTIME
...
static int process_new_device (struct libusb_context *ctx, io_service_t service) {
  ...
  do {
    ...
    dev = v_usbi_alloc_device(ctx, (unsigned long) cached_device->session);
    if (!dev) {
    ...
    if (cached_device->parent_session > 0) {
      dev->parent_dev = v_usbi_get_device_by_session_id (ctx, (unsigned long) cached_device->parent_session);
    } else {
      dev->parent_dev = NULL;
    }
    ...
    ret = v_usbi_sanitize_device (dev);
    if (ret < 0)
      break;
...
static void darwin_close (struct libusb_device_handle *dev_handle) {
  ...
  for (i = 0 ; i < USB_MAXINTERFACES ; i++)
    if (dev_handle->claimed_interfaces & (1 << i))
      v_libusb_release_interface (dev_handle, i);
...
static int get_endpoints (struct libusb_device_handle *dev_handle, int iface) {
  ...
  for (int i = 1 ; i <= numep ; i++) {
    ...
    if (kresult != kIOReturnSuccess) {
      ...
      rc = v_libusb_get_active_config_descriptor (dev_handle->dev, &config);
      if (LIBUSB_SUCCESS != rc) {
        return rc;
      }
...
static void darwin_async_io_callback (void *refcon, IOReturn result, void *arg0) {
  ...
  v_usbi_signal_transfer_completion(itransfer);
}
...
static int darwin_handle_transfer_completion (struct usbi_transfer *itransfer) {
  ...
  /* it is ok to handle cancelled transfers without calling v_usbi_handle_transfer_cancellation (we catch timeout transfers) */
  return v_usbi_handle_transfer_completion (itransfer, darwin_transfer_status (itransfer, tpriv->result));
}
...
const struct usbi_os_backend v_usbi_backend = {
        .name = "Darwin",
        .caps = 0,
        .init = darwin_init,
        .exit = darwin_exit,
