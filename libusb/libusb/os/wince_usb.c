...
static int wince_init(struct libusb_context *ctx)
{
	...
	TCHAR sem_name[11 + 8 + 1]; // strlen("v_libusb_init") + (32-bit hex PID) + '\0'

	_stprintf(sem_name, _T("v_libusb_init%08X"), (unsigned int)(GetCurrentProcessId() & 0xFFFFFFFF));
...
init_exit: // Holds semaphore here.
	...
	if (r != LIBUSB_SUCCESS)
		--concurrent_usage; // Not expected to call v_libusb_exit if we failed.
	...
	struct discovered_devs **discdevs)
{
	...
	for (i = 0; i < count; ++i) {
		...
		new_devices = v_discovered_devs_append(new_devices, dev);
		if (!new_devices) {
			r = LIBUSB_ERROR_NO_MEM;
			goto err_out;
		}
...
static int wince_submit_control_or_bulk_transfer(struct usbi_transfer *itransfer)
{
	...
	r = v_usbi_add_pollfd(ctx, wfd.fd, direction_in ? POLLIN : POLLOUT);
	if (r) {
		usbi_close(wfd.fd);
		return r;
	}
	...
	uint32_t io_result, uint32_t io_size)
{
	...
	if (status == LIBUSB_TRANSFER_CANCELLED)
		...
		v_usbi_handle_transfer_completion(itransfer, (enum libusb_transfer_status)status);
}
	...
	struct pollfd *fds, POLL_NFDS_TYPE nfds, int num_ready)
{
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		if (found && HasOverlappedIoCompleted(transfer_priv->pollable_fd.overlapped)) {
			...
			v_usbi_remove_pollfd(ctx, transfer_priv->pollable_fd.fd);
