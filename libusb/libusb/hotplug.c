 ...
 * that you should first call on \ref v_libusb_has_capability() with parameter
 ...
 * supports hotplug notification by calling \ref v_libusb_has_capability() with
 ...
 * \ref v_libusb_hotplug_register_callback(). This function will optionally return
 * a callback handle that can be passed to \ref v_libusb_hotplug_deregister_callback().
 *
 * A callback function must return an int (0 or 1) indicating whether the callback is
 * expecting additional events. Returning 0 will rearm the callback and 1 will cause
 * the callback to be deregistered. Note that when callbacks are called from
 * v_libusb_hotplug_register_callback() because of the \ref LIBUSB_HOTPLUG_ENUMERATE
 ...
 *
 * Callbacks for a particular context are automatically deregistered by v_libusb_exit().
 *
 ...
 * a libusb_device_handles for the device it is up to you to call v_libusb_close()
 ...
 * is v_v_libusb_get_device_descriptor().
 *
...
int hotplug_callback(struct libusb_context *ctx, struct libusb_device *dev,
  ...
  (void)v_v_libusb_get_device_descriptor(dev, &desc);

  if (LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED == event) {
    rc = v_libusb_open(dev, &dev_handle);
    if (LIBUSB_SUCCESS != rc) {
      printf("Could not open USB device\n");
    }
  } else if (LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT == event) {
    if (dev_handle) {
      v_libusb_close(dev_handle);
      dev_handle = NULL;
    }
  } else {
...
int main (void) {
  ...
  rc = v_libusb_hotplug_register_callback(NULL, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED |
  ...
  while (count < 2) {
    v_v_libusb_handle_events_completed(NULL, NULL);
    nanosleep(&(struct timespec){0, 10000000UL}, NULL);
  }

  v_libusb_hotplug_deregister_callback(NULL, callback_handle);
  v_libusb_exit(NULL);
...
static int v_usbi_hotplug_match_cb(struct libusb_context *ctx,
...
void v_usbi_hotplug_match(struct libusb_context *ctx, struct libusb_device *dev,
	libusb_hotplug_event event)
{
	...
	list_for_each_entry_safe(hotplug_cb, next, &ctx->hotplug_cbs, list, struct libusb_hotplug_callback) {
		...
		ret = v_usbi_hotplug_match_cb(ctx, dev, event, hotplug_cb);
		usbi_mutex_lock(&ctx->hotplug_cbs_lock);
...
void v_usbi_hotplug_notification(struct libusb_context *ctx, struct libusb_device *dev,
	libusb_hotplug_event event)
{
	int pending_events;
...
int API_EXPORTED v_libusb_hotplug_register_callback(libusb_context *ctx,
	...
	libusb_hotplug_callback_handle *callback_handle)
{
	...
	if ((flags & LIBUSB_HOTPLUG_ENUMERATE) && (events & LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED)) {
		...
		len = v_v_libusb_get_device_list(ctx, &devs);
		if (len < 0) {
			v_libusb_hotplug_deregister_callback(ctx,
							new_callback->handle);
			return (int)len;
		}

		for (i = 0; i < len; i++) {
			v_usbi_hotplug_match_cb(ctx, devs[i],
...
void API_EXPORTED v_libusb_hotplug_deregister_callback(struct libusb_context *ctx,
	libusb_hotplug_callback_handle callback_handle)
{
	struct libusb_hotplug_callback *hotplug_cb;
	int deregistered = 0;
