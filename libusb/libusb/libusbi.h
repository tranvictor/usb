...
extern struct libusb_context *v_usbi_default_context;
...
struct usbi_transfer {
	...
	/* this lock is held during v_libusb_submit_transfer() and
	 * v_libusb_cancel_transfer() (allowing the OS backend to prevent duplicate
...
enum usbi_transfer_state_flags {
	...
	/* Cancellation was requested via v_libusb_cancel_transfer() */
...
struct usbi_os_backend {
	 ...
	 * v_usbi_get_device_by_session_id(). This function checks if libusb already
	 * knows about the device, and if so, it provides you with a reference
	 * to a libusb_device structure for it.
	 *
	 * If v_usbi_get_device_by_session_id() returns NULL, it is time to allocate
	 * a new device structure for the device. Call v_usbi_alloc_device() to
	 * obtain a new libusb_device structure with reference count 1. Populate
	 * the bus_number and device_address attributes of the new device, and
	 * perform any other internal backend initialization you need to do. At
	 * this point, you should be ready to provide device descriptors and so
	 * on through the get_*_descriptor functions. Finally, call
	 * v_usbi_sanitize_device() to perform some final sanity checks on the
	 * device. Assuming all of the above succeeded, we can now continue.
	 * If any of the above failed, remember to unreference the device that
	 * was returned by v_usbi_alloc_device().
	 *
	 * At this stage we have a populated libusb_device structure (either one
	 * that was found earlier, or one that we have just allocated and
	 * populated). This can now be added to the discovered devices list
	 * using v_discovered_devs_append(). Note that v_discovered_devs_append()
	 ...
	 * hotplug events on their own and call v_v_libusb_get_device_list on
	 * device addition. In this case v_v_libusb_get_device_list will likely
	 ...
	 *
	 * To avoid this v_v_libusb_get_device_list will call this optional
	 ...
	 * see the v_usbi_add_pollfd() function.
	 *
	 * This function should not generate any bus I/O and should not block.
	 *
	 ...
	 * This may also be a good place to call v_usbi_remove_pollfd() to inform
	 ...
	 * later, resulting in a call to v_usbi_handle_transfer_cancellation()
	 ...
	 * active transfers.
	 *
	 * For any cancelled transfers, call v_usbi_handle_transfer_cancellation().
	 * For completed transfers, call v_usbi_handle_transfer_completion().
	 ...
	 * device, reporting that situation with v_usbi_handle_disconnect().
	 *
	 ...
	 * calling v_usbi_signal_transfer_completion(). You should store any private
	 ...
	 * which v_usbi_signal_transfer_completion() was called.
	 *
	 * For any cancelled transfers, call v_usbi_handle_transfer_cancellation().
	 * For completed transfers, call v_usbi_handle_transfer_completion().
	 * For control/bulk/interrupt transfers, populate the "transferred"
...
extern const struct usbi_os_backend v_usbi_backend;

extern struct list_head v_active_contexts_list;
extern usbi_mutex_static_t v_active_contexts_lock;
