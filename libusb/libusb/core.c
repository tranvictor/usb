...
struct libusb_context *v_usbi_default_context = NULL;
...
usbi_mutex_static_t v_active_contexts_lock = USBI_MUTEX_INITIALIZER;
struct list_head v_active_contexts_list;
 ...
 *
 * The v_libusb_reset_device() function allows you to reset a device. If your
 ...
 *
 * - Configuration activation (v_libusb_set_configuration())
 * - Interface/alternate setting activation (v_libusb_set_interface_alt_setting())
 * - Releasing of interfaces (v_libusb_release_interface())
 * - Clearing of halt/stall condition (v_libusb_clear_halt())
 * - Device resets (v_libusb_reset_device())
 *
 ...
 *
 * The obvious solution is to add a call to v_libusb_set_configuration() early
 * on during your device initialization routines, but there are caveats to
 * be aware of:
 * -# If the device is already in the desired configuration, calling
 *    v_libusb_set_configuration() using the same configuration value will cause
 ...
 *    perform the lightweight device reset, so v_libusb_set_configuration() will
 ...
 *    v_libusb_set_configuration() may succeed.
 *
...
\code
cfg = -1;
v_libusb_get_configuration(dev, &cfg);
if (cfg != desired)
	v_libusb_set_configuration(dev, desired);
\endcode
 *
 ...
 * <em>after</em> the v_libusb_get_configuration() call.
 *
 * Even in cases where v_libusb_set_configuration() succeeds, consider that other
 * applications or drivers may change configuration after your application
 * calls v_libusb_set_configuration().
 *
 ...
 * users can continue using libusb after one of them calls v_libusb_exit(), etc.
 *
 * This is made possible through libusb's <em>context</em> concept. When you
 * call v_libusb_init(), you are (optionally) given a context. You can then pass
 ...
 * is created the first time a process calls v_libusb_init() when no other
 * context is alive. Contexts are destroyed during v_libusb_exit().
 *
 * The default context is reference-counted and can be shared. That means that
 * if v_libusb_init(NULL) is called twice within the same process, the two
 * users end up sharing the same context. The deinitialization and freeing of
 * the default context will only happen when the last user calls v_libusb_exit().
 ...
 */
  ...
  *
  * \section Functions
  * - v_libusb_alloc_streams()
  * - v_libusb_alloc_transfer()
  * - v_libusb_attach_kernel_driver()
  * - v_libusb_bulk_transfer()
  * - v_libusb_cancel_transfer()
  * - v_libusb_claim_interface()
  * - v_libusb_clear_halt()
  * - v_libusb_close()
  * - v_libusb_control_transfer()
  * - v_libusb_control_transfer_get_data()
  * - v_libusb_control_transfer_get_setup()
  * - libusb_cpu_to_le16()
  * - v_libusb_detach_kernel_driver()
  * - v_libusb_dev_mem_alloc()
  * - v_libusb_dev_mem_free()
  * - v_libusb_error_name()
  * - v_libusb_event_handler_active()
  * - v_libusb_event_handling_ok()
  * - v_libusb_exit()
  ...
  * - v_libusb_free_bos_descriptor()
  * - v_libusb_free_config_descriptor()
  * - v_libusb_free_container_id_descriptor()
  * - v_libusb_free_device_list()
  * - v_libusb_free_pollfds()
  * - v_libusb_free_ss_endpoint_companion_descriptor()
  * - v_libusb_free_ss_usb_device_capability_descriptor()
  * - v_libusb_free_streams()
  * - v_libusb_free_transfer()
  * - v_libusb_free_usb_2_0_extension_descriptor()
  * - v_libusb_get_active_config_descriptor()
  * - v_libusb_get_bos_descriptor()
  * - v_libusb_get_bus_number()
  * - v_libusb_get_config_descriptor()
  * - v_v_libusb_get_config_descriptor_by_value()
  * - v_libusb_get_configuration()
  * - v_libusb_get_container_id_descriptor()
  * - libusb_get_descriptor()
  * - v_libusb_get_device()
  * - v_v_libusb_get_device_address()
  * - v_v_libusb_get_device_descriptor()
  * - v_v_libusb_get_device_list()
  * - v_v_libusb_get_device_speed()
  * - libusb_get_iso_packet_buffer()
  * - libusb_get_iso_packet_buffer_simple()
  * - v_libusb_get_max_iso_packet_size()
  * - v_libusb_get_max_packet_size()
  * - v_libusb_get_next_timeout()
  * - v_libusb_get_parent()
  * - v_libusb_get_pollfds()
  * - v_libusb_get_port_number()
  * - v_v_libusb_get_port_numbers()
  * - v_libusb_get_port_path()
  * - v_libusb_get_ss_endpoint_companion_descriptor()
  * - v_libusb_get_ss_usb_device_capability_descriptor()
  * - libusb_get_string_descriptor()
  * - v_libusb_get_string_descriptor_ascii()
  * - v_libusb_get_usb_2_0_extension_descriptor()
  * - v_libusb_get_version()
  * - v_libusb_handle_events()
  * - v_v_libusb_handle_events_completed()
  * - v_v_libusb_handle_events_locked()
  * - v_v_libusb_handle_events_timeout()
  * - v_v_v_libusb_handle_events_timeout_completed()
  * - v_libusb_has_capability()
  * - v_libusb_hotplug_deregister_callback()
  * - v_libusb_hotplug_register_callback()
  * - v_libusb_init()
  * - v_libusb_interrupt_event_handler()
  * - v_libusb_interrupt_transfer()
  * - v_libusb_kernel_driver_active()
  * - v_libusb_lock_events()
  * - v_libusb_lock_event_waiters()
  * - v_libusb_open()
  * - v_v_libusb_open_device_with_vid_pid()
  * - v_libusb_pollfds_handle_timeouts()
  * - v_libusb_ref_device()
  * - v_libusb_release_interface()
  * - v_libusb_reset_device()
  * - v_libusb_set_auto_detach_kernel_driver()
  * - v_libusb_set_configuration()
  * - v_libusb_set_debug()
  * - v_libusb_set_interface_alt_setting()
  * - libusb_set_iso_packet_lengths()
  * - libusb_set_option()
  * - v_libusb_setlocale()
  * - v_libusb_set_pollfd_notifiers()
  * - v_libusb_strerror()
  * - v_libusb_submit_transfer()
  * - v_libusb_transfer_get_stream_id()
  * - v_libusb_transfer_set_stream_id()
  * - v_libusb_try_lock_events()
  * - v_libusb_unlock_events()
  * - v_libusb_unlock_event_waiters()
  * - v_libusb_unref_device()
  * - v_libusb_wait_for_event()
  *
  * \section Structures
...
ssize_t cnt = v_v_libusb_get_device_list(NULL, &list);
ssize_t i = 0;
int err = 0;
if (cnt < 0)
	error();
...
if (found) {
	...
	err = v_libusb_open(found, &handle);
	if (err)
		error();
	// etc
}

v_libusb_free_device_list(list, 1);
\endcode
 *
 * The two important points:
 * - You asked v_libusb_free_device_list() to unreference the devices (2nd
 *   parameter)
 ...
 *
 * The v_v_libusb_get_device_list() function can be used to obtain a list of
 ...
 * libusb to open the device using the v_libusb_open() function. Assuming
 ...
 *
 * Device discovery (i.e. calling v_v_libusb_get_device_list()) returns a
 ...
 *
 * New devices presented by the v_v_libusb_get_device_list() function all have a
 * reference count of 1. You can increase and decrease reference count using
 * v_libusb_ref_device() and v_libusb_unref_device(). A device is destroyed when
 * its reference count reaches 0.
 *
 * With the above information in mind, the process of opening a device can
 * be viewed as follows:
 * -# Discover devices using v_v_libusb_get_device_list().
 * -# Choose the device that you want to operate, and call v_libusb_open().
 ...
 *
 * For convenience, the v_libusb_free_device_list() function includes a
 * parameter to optionally unreference all the devices in the list before
 * freeing the list itself. This combines steps 3 and 4 above.
 *
 * As an implementation detail, v_libusb_open() actually adds a reference to
 * the device in question. This is because the device remains available
 * through the handle via v_libusb_get_device(). The reference is deleted during
 * v_libusb_close().
 */
...
static void discovered_devs_free(struct discovered_devs *discdevs)
{
	...
	for (i = 0; i < discdevs->len; i++)
		v_libusb_unref_device(discdevs->devices[i]);
...
struct discovered_devs *v_discovered_devs_append(
	struct discovered_devs *discdevs, struct libusb_device *dev)
{
	...
	if (len < discdevs->capacity) {
		discdevs->devices[len] = v_libusb_ref_device(dev);
		discdevs->len++;
		return discdevs;
	}
...
struct libusb_device *v_usbi_alloc_device(struct libusb_context *ctx,
	unsigned long session_id)
{
	size_t priv_size = v_usbi_backend.device_priv_size;
	...
	if (!v_libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
		v_usbi_connect_device (dev);
	}

	return dev;
}

void v_usbi_connect_device(struct libusb_device *dev)
{
	...
	if (v_libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG) && dev->ctx->hotplug_msgs.next) {
		v_usbi_hotplug_notification(ctx, dev, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED);
	}
}

void v_usbi_disconnect_device(struct libusb_device *dev)
{
	struct libusb_context *ctx = DEVICE_CTX(dev);
	...
	/* Signal that an event has occurred for this device if we support hotplug AND
	 * the hotplug message list is ready. This prevents an event from getting raised
	 * during initial enumeration. v_libusb_handle_events will take care of dereferencing
	 * the device. */
	if (v_libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG) && dev->ctx->hotplug_msgs.next) {
		v_usbi_hotplug_notification(ctx, dev, LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT);
	}
}
...
int v_usbi_sanitize_device(struct libusb_device *dev)
{
	int r;
	uint8_t num_configurations;

	r = v_usbi_device_cache_descriptor(dev);
	if (r < 0)
		return r;
 ...
 * NULL otherwise. */
struct libusb_device *v_usbi_get_device_by_session_id(struct libusb_context *ctx,
	unsigned long session_id)
{
 ...
 * them, and then free the list with v_libusb_free_device_list(). Note that
 * v_libusb_free_device_list() can unref all the devices for you. Be careful
 ...
 */
ssize_t API_EXPORTED v_v_libusb_get_device_list(libusb_context *ctx,
	libusb_device ***list)
{
	...
	if (v_libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
		...
		if (v_usbi_backend.hotplug_poll)
			v_usbi_backend.hotplug_poll();

		usbi_mutex_lock(&ctx->usb_devs_lock);
		list_for_each_entry(dev, &ctx->usb_devs, list, struct libusb_device) {
			discdevs = v_discovered_devs_append(discdevs, dev);
		...
		r = v_usbi_backend.get_device_list(ctx, &discdevs);
	}
 ...
 * v_v_libusb_get_device_list(). If the unref_devices parameter is set, the
 * reference count of each device in the list is decremented by 1.
 * \param list the list to free
 * \param unref_devices whether to unref the devices in the list
 */
void API_EXPORTED v_libusb_free_device_list(libusb_device **list,
	int unref_devices)
{
	if (!list)
		return;
 ...
 */
uint8_t API_EXPORTED v_libusb_get_bus_number(libusb_device *dev)
{
 ...
 */
uint8_t API_EXPORTED v_libusb_get_port_number(libusb_device *dev)
{
 ...
 */
int API_EXPORTED v_v_libusb_get_port_numbers(libusb_device *dev,
...
/** \ingroup libusb_dev
 * Deprecated please use v_v_libusb_get_port_numbers instead.
 */
int API_EXPORTED v_libusb_get_port_path(libusb_context *ctx, libusb_device *dev,
	uint8_t* port_numbers, uint8_t port_numbers_len)
{
	UNUSED(ctx);

	return v_v_libusb_get_port_numbers(dev, port_numbers, port_numbers_len);
}
 ...
 * You should issue a \ref v_v_libusb_get_device_list() before calling this
 * function and make sure that you only access the parent before issuing
 * \ref v_libusb_free_device_list(). The reason is that libusb currently does
 * not maintain a permanent list of device instances, and therefore can
 * only guarantee that parents are fully instantiated within a 
 * v_v_libusb_get_device_list() - v_libusb_free_device_list() block.
 */
DEFAULT_VISIBILITY
libusb_device * LIBUSB_CALL v_libusb_get_parent(libusb_device *dev)
{
 ...
 */
uint8_t API_EXPORTED v_v_libusb_get_device_address(libusb_device *dev)
{
 ...
 */
int API_EXPORTED v_v_libusb_get_device_speed(libusb_device *dev)
{
	return dev->speed;
}
 ...
 * want v_libusb_get_max_iso_packet_size() instead.
 *
 * \param dev a device
 ...
 */
int API_EXPORTED v_libusb_get_max_packet_size(libusb_device *dev,
	unsigned char endpoint)
{
	...
	r = v_libusb_get_active_config_descriptor(dev, &config);
	if (r < 0) {
...
out:
	v_libusb_free_config_descriptor(config);
	return r;
}
 ...
 * \returns LIBUSB_ERROR_OTHER on other failure
 */
int API_EXPORTED v_libusb_get_max_iso_packet_size(libusb_device *dev,
	unsigned char endpoint)
{
	struct libusb_config_descriptor *config;
	const struct libusb_endpoint_descriptor *ep;
 ...
 */
DEFAULT_VISIBILITY
libusb_device * LIBUSB_CALL v_libusb_ref_device(libusb_device *dev)
{
	usbi_mutex_lock(&dev->lock);
	dev->refcnt++;
	usbi_mutex_unlock(&dev->lock);
 ...
 */
void API_EXPORTED v_libusb_unref_device(libusb_device *dev)
{
	int refcnt;
	...
	if (refcnt == 0) {
		...
		v_libusb_unref_device(dev->parent_dev);

		if (v_usbi_backend.destroy_device)
			v_usbi_backend.destroy_device(dev);

		if (!v_libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
			/* backend does not support hotplug */
			v_usbi_disconnect_device(dev);
		}
 ...
 */
int v_usbi_signal_event(struct libusb_context *ctx)
{
 ...
 * interrupted.
 */
int v_usbi_clear_event(struct libusb_context *ctx)
{
	unsigned char dummy;
	ssize_t r;
 ...
 * available to you through v_libusb_get_device(). This reference is removed
 * during v_libusb_close().
 *
 ...
 */
int API_EXPORTED v_libusb_open(libusb_device *dev,
	libusb_device_handle **dev_handle)
{
	struct libusb_context *ctx = DEVICE_CTX(dev);
	struct libusb_device_handle *_dev_handle;
	size_t priv_size = v_usbi_backend.device_handle_priv_size;
	int r;
 ...
 * application - it allows you to avoid calling v_v_libusb_get_device_list() and
...
DEFAULT_VISIBILITY
libusb_device_handle * LIBUSB_CALL v_v_libusb_open_device_with_vid_pid(
	libusb_context *ctx, uint16_t vendor_id, uint16_t product_id)
{
	...
	if (v_v_libusb_get_device_list(ctx, &devs) < 0)
		return NULL;

	while ((dev = devs[i++]) != NULL) {
		struct libusb_device_descriptor desc;
		r = v_v_libusb_get_device_descriptor(dev, &desc);
		if (r < 0)
			goto out;
	...
	if (found) {
		r = v_libusb_open(found, &dev_handle);
		if (r < 0)
			dev_handle = NULL;
	}

out:
	v_libusb_free_device_list(devs, 1);
	return dev_handle;
}
	...
	struct libusb_device_handle *dev_handle)
{
	...
	v_usbi_backend.close(dev_handle);
	v_libusb_unref_device(dev_handle->dev);
 ...
 * v_libusb_open() on the given device.
 *
 * This is a non-blocking function; no requests are sent over the bus.
 *
 * \param dev_handle the device handle to close
 */
void API_EXPORTED v_libusb_close(libusb_device_handle *dev_handle)
{
	...
	/* Similarly to v_libusb_open(), we want to interrupt all event handlers
	...
	if (!handling_events) {
		...
		ctx->device_close++;
		if (!pending_events)
			v_usbi_signal_event(ctx);
		usbi_mutex_unlock(&ctx->event_data_lock);

		/* take event handling lock */
		v_libusb_lock_events(ctx);
	}
	...
	if (!handling_events) {
		...
		pending_events = usbi_pending_events(ctx);
		if (!pending_events)
			v_usbi_clear_event(ctx);
		usbi_mutex_unlock(&ctx->event_data_lock);

		/* Release event handling lock and wake up event waiters */
		v_libusb_unlock_events(ctx);
	}
}
 ...
 */
DEFAULT_VISIBILITY
libusb_device * LIBUSB_CALL v_libusb_get_device(libusb_device_handle *dev_handle)
{
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED v_libusb_get_configuration(libusb_device_handle *dev_handle,
	int *config)
{
	int r = LIBUSB_ERROR_NOT_SUPPORTED;

	usbi_dbg("");
	if (v_usbi_backend.get_configuration)
		r = v_usbi_backend.get_configuration(dev_handle, config);

	if (r == LIBUSB_ERROR_NOT_SUPPORTED) {
		uint8_t tmp = 0;
		usbi_dbg("falling back to control message");
		r = v_libusb_control_transfer(dev_handle, LIBUSB_ENDPOINT_IN,
 ...
 * interfaces.
 *
 * Alternatively you can call v_libusb_release_interface() first. Note if you
 ...
 * \returns LIBUSB_ERROR_NO_DEVICE if the device has been disconnected
 * \returns another LIBUSB_ERROR code on other failure
 * \see v_libusb_set_auto_detach_kernel_driver()
 */
int API_EXPORTED v_libusb_set_configuration(libusb_device_handle *dev_handle,
	int configuration)
{
	usbi_dbg("configuration %d", configuration);
	return v_usbi_backend.set_configuration(dev_handle, configuration);
}
 ...
 * \see v_libusb_set_auto_detach_kernel_driver()
 */
int API_EXPORTED v_libusb_claim_interface(libusb_device_handle *dev_handle,
	int interface_number)
{
	...
	r = v_usbi_backend.claim_interface(dev_handle, interface_number);
	if (r == 0)
...
/** \ingroup libusb_dev
 * Release an interface previously claimed with v_libusb_claim_interface(). You
 ...
 * \see v_libusb_set_auto_detach_kernel_driver()
 */
int API_EXPORTED v_libusb_release_interface(libusb_device_handle *dev_handle,
	int interface_number)
{
	int r;
	...
	r = v_usbi_backend.release_interface(dev_handle, interface_number);
	if (r == 0)
		dev_handle->claimed_interfaces &= ~(1 << interface_number);
 ...
 * been previously claimed with v_libusb_claim_interface().
 *
 * You should always use this function rather than formulating your own
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED v_libusb_set_interface_alt_setting(libusb_device_handle *dev_handle,
	int interface_number, int alternate_setting)
{
	...
	return v_usbi_backend.set_interface_altsetting(dev_handle, interface_number,
		alternate_setting);
}
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED v_libusb_clear_halt(libusb_device_handle *dev_handle,
	unsigned char endpoint)
{
	...
	return v_usbi_backend.clear_halt(dev_handle, endpoint);
}
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED v_libusb_reset_device(libusb_device_handle *dev_handle)
{
	usbi_dbg("");
	if (!dev_handle->dev->attached)
		return LIBUSB_ERROR_NO_DEVICE;

	return v_usbi_backend.reset_device(dev_handle);
}
 ...
 * If v_libusb_alloc_streams() returns with a value of N, you may use stream ids
 * 1 to N.
 *
 ...
 */
int API_EXPORTED v_libusb_alloc_streams(libusb_device_handle *dev_handle,
	uint32_t num_streams, unsigned char *endpoints, int num_endpoints)
{
	...
	if (v_usbi_backend.alloc_streams)
		return v_usbi_backend.alloc_streams(dev_handle, num_streams, endpoints,
						   num_endpoints);
	else
...
/** \ingroup libusb_asyncio
 * Free usb bulk streams allocated with v_libusb_alloc_streams().
 *
 ...
 */
int API_EXPORTED v_libusb_free_streams(libusb_device_handle *dev_handle,
	unsigned char *endpoints, int num_endpoints)
{
	...
	if (v_usbi_backend.free_streams)
		return v_usbi_backend.free_streams(dev_handle, endpoints,
						  num_endpoints);
	else
		return LIBUSB_ERROR_NOT_SUPPORTED;
}
 ...
 * freed with \ref v_libusb_dev_mem_free. Specifically, this means that the
 ...
 */
DEFAULT_VISIBILITY
unsigned char * LIBUSB_CALL v_libusb_dev_mem_alloc(libusb_device_handle *dev_handle,
        size_t length)
{
	if (!dev_handle->dev->attached)
		return NULL;

	if (v_usbi_backend.dev_mem_alloc)
		return v_usbi_backend.dev_mem_alloc(dev_handle, length);
	else
		return NULL;
}

/** \ingroup libusb_asyncio
 * Free device memory allocated with v_libusb_dev_mem_alloc().
 *
 * \param dev_handle a device handle
 ...
 * \returns LIBUSB_SUCCESS, or a LIBUSB_ERROR code on failure
 */
int API_EXPORTED v_libusb_dev_mem_free(libusb_device_handle *dev_handle,
	unsigned char *buffer, size_t length)
{
	if (v_usbi_backend.dev_mem_free)
		return v_usbi_backend.dev_mem_free(dev_handle, buffer, length);
	else
		return LIBUSB_ERROR_NOT_SUPPORTED;
}
 ...
 * is not available
 * \returns another LIBUSB_ERROR code on other failure
 * \see v_libusb_detach_kernel_driver()
 */
int API_EXPORTED v_libusb_kernel_driver_active(libusb_device_handle *dev_handle,
	int interface_number)
{
	usbi_dbg("interface %d", interface_number);
	...
	if (v_usbi_backend.kernel_driver_active)
		return v_usbi_backend.kernel_driver_active(dev_handle, interface_number);
	else
		return LIBUSB_ERROR_NOT_SUPPORTED;
}
 ...
 * is not available
 * \returns another LIBUSB_ERROR code on other failure
 * \see v_libusb_kernel_driver_active()
 */
int API_EXPORTED v_libusb_detach_kernel_driver(libusb_device_handle *dev_handle,
	int interface_number)
{
	usbi_dbg("interface %d", interface_number);
	...
	if (v_usbi_backend.detach_kernel_driver)
		return v_usbi_backend.detach_kernel_driver(dev_handle, interface_number);
	else
		return LIBUSB_ERROR_NOT_SUPPORTED;
}
 ...
 * using v_libusb_detach_kernel_driver(). This call is only effective on
 ...
 * \see v_libusb_kernel_driver_active()
 */
int API_EXPORTED v_libusb_attach_kernel_driver(libusb_device_handle *dev_handle,
	int interface_number)
{
	usbi_dbg("interface %d", interface_number);
	...
	if (v_usbi_backend.attach_kernel_driver)
		return v_usbi_backend.attach_kernel_driver(dev_handle, interface_number);
	else
		return LIBUSB_ERROR_NOT_SUPPORTED;
}
 ...
 * \returns LIBUSB_ERROR_NOT_SUPPORTED on platforms where the functionality
 * is not available
 * \see v_libusb_claim_interface()
 * \see v_libusb_release_interface()
 * \see v_libusb_set_configuration()
 */
int API_EXPORTED v_libusb_set_auto_detach_kernel_driver(
	libusb_device_handle *dev_handle, int enable)
{
	if (!(v_usbi_backend.caps & USBI_CAP_SUPPORTS_DETACH_KERNEL_DRIVER))
		return LIBUSB_ERROR_NOT_SUPPORTED;
 ...
 */
void API_EXPORTED v_libusb_set_debug(libusb_context *ctx, int level)
{
	...
	enum libusb_option option, ...)
{
	...
	case LIBUSB_OPTION_USE_USBDK:
		if (v_usbi_backend.set_option)
			r = v_usbi_backend.set_option(ctx, option, ap);
		else
 ...
 * \see libusb_contexts
 */
int API_EXPORTED v_libusb_init(libusb_context **context)
{
	struct libusb_device *dev, *next;
	size_t priv_size = v_usbi_backend.context_priv_size;
	struct libusb_context *ctx;
	...
	if (!timestamp_origin.tv_sec) {
		v_usbi_backend.clock_gettime(USBI_CLOCK_REALTIME, &timestamp_origin);
	}

	if (!context && v_usbi_default_context) {
	...
	if (!v_usbi_default_context) {
		v_usbi_default_context = ctx;
		default_context_refcnt++;
	...
	if (first_init) {
		first_init = 0;
		list_init (&v_active_contexts_list);
	}
	list_add (&ctx->list, &v_active_contexts_list);
	usbi_mutex_static_unlock(&v_active_contexts_lock);

	if (v_usbi_backend.init) {
		r = v_usbi_backend.init(ctx);
		if (r)
			goto err_free_ctx;
	}

	r = v_usbi_io_init(ctx);
	if (r < 0)
...
err_backend_exit:
	if (v_usbi_backend.exit)
		v_usbi_backend.exit(ctx);
err_free_ctx:
	if (ctx == v_usbi_default_context) {
		v_usbi_default_context = NULL;
		default_context_refcnt--;
	}
 ...
 */
void API_EXPORTED v_libusb_exit(struct libusb_context *ctx)
{
	struct libusb_device *dev, *next;
	...
	if (v_libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
		...
		if (list_empty(&ctx->open_devs))
			v_v_libusb_handle_events_timeout(ctx, &tv);
 ...
 * This call should be performed after \ref v_libusb_init(), to ensure the
 * backend has updated its capability set.
 *
 * \param capability the \ref libusb_capability to check for
 * \returns nonzero if the running library has the capability, 0 otherwise
 */
int API_EXPORTED v_libusb_has_capability(uint32_t capability)
{
	switch (capability) {
	case LIBUSB_CAP_HAS_CAPABILITY:
		return 1;
	case LIBUSB_CAP_HAS_HOTPLUG:
		return !(v_usbi_backend.get_device_list);
	case LIBUSB_CAP_HAS_HID_ACCESS:
		return (v_usbi_backend.caps & USBI_CAP_HAS_HID_ACCESS);
	case LIBUSB_CAP_SUPPORTS_DETACH_KERNEL_DRIVER:
		return (v_usbi_backend.caps & USBI_CAP_SUPPORTS_DETACH_KERNEL_DRIVER);
	}
	return 0;
}
	...
	const char *function, const char *format, va_list args)
{
	...
	v_usbi_backend.clock_gettime(USBI_CLOCK_REALTIME, &now);
	...
	if (global_debug) {
		header_len = snprintf(buf, sizeof(buf),
			...
			(int)now.tv_sec, (int)(now.tv_nsec / 1000L), v_usbi_get_tid(), prefix, function);
	} else {
		header_len = snprintf(buf, sizeof(buf),
 ...
 */
DEFAULT_VISIBILITY const char * LIBUSB_CALL v_libusb_error_name(int error_code)
{
	switch (error_code) {
 ...
 */
DEFAULT_VISIBILITY
const struct libusb_version * LIBUSB_CALL v_libusb_get_version(void)
{
