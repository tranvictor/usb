...
int actual_length;
int r = v_libusb_bulk_transfer(dev_handle, LIBUSB_ENDPOINT_IN, data, sizeof(data), &actual_length, 0);
 ...
 * inside v_libusb_bulk_transfer() until the transaction has completed. If it
 ...
 *
 * Allocation is done with the v_libusb_alloc_transfer() function. You must use
 ...
 * v_libusb_submit_transfer(). This function returns immediately but can be
 ...
 * below). Transfers are deallocated with v_libusb_free_transfer().
 *
 ...
 * done by calling the v_libusb_cancel_transfer() function.
 *
 * v_libusb_cancel_transfer() is asynchronous/non-blocking in itself. When the
 ...
 * correct offset, you may wish to use the v_libusb_control_transfer_get_data()
 * and v_libusb_control_transfer_get_setup() functions within your transfer
 * callback.
 *
 ...
 * v_libusb_alloc_transfer() with an appropriate number of isochronous packets.
 *
 ...
 * v_libusb_alloc_transfer() does not set either of these fields for you, given
 ...
 *
 * - v_libusb_get_max_iso_packet_size() is an easy way to determine the max
 ...
 * v_libusb_submit_transfer() will return
 ...
 *
 * This gives rise to the v_libusb_handle_events() function which your
 * application must call into when libusb has work do to. This gives libusb
 * the opportunity to reap pending transfers, invoke callbacks, etc.
 *
 * There are 2 different approaches to dealing with v_libusb_handle_events:
 *
 * -# Repeatedly call v_libusb_handle_events() in blocking mode from a dedicated
 *    thread.
 ...
 * event_thread_run variable to 0, and after that v_libusb_handle_events() needs
 * to return control to event_thread_func. But unless some event happens,
 * v_libusb_handle_events() will not return.
 *
 ...
 * thread until after their first call to v_libusb_open(), and should stop the
...
void my_close_handle(libusb_device_handle *dev_handle)
{
    ...
    v_libusb_close(dev_handle); // This wakes up v_libusb_handle_events()
 ...
 * after having successfully called v_libusb_hotplug_register_callback(), and
 * should stop the thread at program exit as follows:
\code
void my_v_libusb_exit(void)
{
    event_thread_run = 0;
    v_libusb_hotplug_deregister_callback(ctx, hotplug_cb_handle); // This wakes up v_libusb_handle_events()
    pthread_join(event_thread);
    v_libusb_exit(ctx);
}
\endcode
 ...
 * sets of file descriptors or handling timeouts. v_libusb_handle_events() will
 * handle those details internally.
 *
 * \section libusb_pollmain The more advanced option
 *
 * \note This functionality is currently only available on Unix-like platforms.
 * On Windows, v_libusb_get_pollfds() simply returns NULL. Applications which
 ...
 * v_v_libusb_handle_events_timeout() in non-blocking mode.
 *
 * What's more, libusb may also need to handle events at specific moments in
 * time. No file descriptor activity is generated at these times, so your
 * own application needs to be continually aware of when the next one of these
 * moments occurs (through calling v_libusb_get_next_timeout()), and then it
 * needs to call v_v_libusb_handle_events_timeout() in non-blocking mode when
...
v_libusb_get_pollfds(ctx)
while (user has not requested application exit) {
	v_libusb_get_next_timeout(ctx);
	poll(on libusb file descriptors plus any other event sources of interest,
		using a timeout no larger than the value libusb just suggested)
	if (poll() indicated activity on libusb file descriptors)
		v_v_libusb_handle_events_timeout(ctx, &zero_tv);
 ...
 *
 * Under these configurations, v_libusb_get_next_timeout() will \em always return
 ...
 *
 * You can use the v_libusb_pollfds_handle_timeouts() function to do a runtime
 * check to see if it is safe to ignore the time-based event complications.
 * If your application has taken the shortcut of ignoring libusb's next timeout
 * in your main loop, then you are advised to check the return value of
 * v_libusb_pollfds_handle_timeouts() during application startup, and to abort
 ...
 * call v_libusb_get_pollfds(), you can set up notification functions for when
 * the file descriptor set changes using v_libusb_set_pollfd_notifiers().
 *
 ...
 * some headaches.
 *
 * The events lock, event waiters lock, and v_v_libusb_handle_events_locked()
...
void myfunc() {
	...
	transfer = v_libusb_alloc_transfer(0);
	libusb_fill_control_setup(buffer,
		LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT, 0x04, 0x01, 0, 0);
	libusb_fill_control_transfer(transfer, dev, buffer, cb, &completed, 1000);
	v_libusb_submit_transfer(transfer);
 ...
 * This includes v_libusb_handle_events() and its variants, and all the
 * synchronous I/O functions - libusb hides this headache from you.
 *
 * \section Using v_libusb_handle_events() from multiple threads
 *
 * Even when only using v_libusb_handle_events() and synchronous I/O functions,
 * you can still have a race condition. You might be tempted to solve the
 * above with v_libusb_handle_events() like so:
 *
\code
	v_libusb_submit_transfer(transfer);
 ...
 * v_libusb_handle_events() acquiring the events lock, so another thread
 ...
 * v_libusb_handle_events() without worrying about locking. This is why
 * libusb-1.0.9 introduces the new v_v_v_libusb_handle_events_timeout_completed()
 * and v_v_libusb_handle_events_completed() functions, which handles doing the
...
\code
	...
	while (!completed) {
		v_v_libusb_handle_events_completed(ctx, &completed);
	}
	printf("completed!");
\endcode
 *
 ...
 * using v_libusb_lock_events(). You must release the lock as soon as you have
 * aborted your poll()/select() loop, using v_libusb_unlock_events().
 *
 ...
 * obtain the <em>event waiters</em> lock using v_libusb_lock_event_waiters().
 * You then re-check that some other thread is still handling events, and if
 * so, you call v_libusb_wait_for_event().
 *
 * v_libusb_wait_for_event() puts your application to sleep until an event
...
\code
retry:
if (v_libusb_try_lock_events(ctx) == 0) {
	// we obtained the event lock: do our own event handling
	while (!completed) {
		if (!v_libusb_event_handling_ok(ctx)) {
			v_libusb_unlock_events(ctx);
			goto retry;
		}
		poll(libusb file descriptors, 120*1000);
		if (poll indicates activity)
			v_v_libusb_handle_events_locked(ctx, 0);
	}
	v_libusb_unlock_events(ctx);
} else {
	// another thread is doing event handling. wait for it to signal us that
	// an event has completed
	v_libusb_lock_event_waiters(ctx);

	while (!completed) {
		// now that we have the event waiters lock, double check that another
		// thread is still handling events for us. (it may have ceased handling
		// events in the time it took us to reach this point)
		if (!v_libusb_event_handler_active(ctx)) {
			// whoever was handling events is no longer doing so, try again
			v_libusb_unlock_event_waiters(ctx);
			goto retry;
		}

		v_libusb_wait_for_event(ctx, NULL);
	}
	v_libusb_unlock_event_waiters(ctx);
}
printf("completed!\n");
 ...
 * multiple event waiters, because v_libusb_wait_for_event() actually drops
 ...
 * -# v_libusb_try_lock_events() is a non-blocking function which attempts
 *    to acquire the events lock but returns a failure code if it is contended.
 * -# v_libusb_event_handling_ok() checks that libusb is still happy for your
 *    thread to be performing event handling. Sometimes, libusb needs to
 *    interrupt the event handler, and this is how you can check if you have
 *    been interrupted. If this function returns 0, the correct behaviour is
 *    for you to give up the event handling lock, and then to repeat the cycle.
 *    The following v_libusb_try_lock_events() will fail, so you will become an
 *    events waiter. For more information on this, read \ref fullstory below.
 * -# v_v_libusb_handle_events_locked() is a variant of
 *    v_v_libusb_handle_events_timeout() that you can call while holding the
 *    events lock. v_v_libusb_handle_events_timeout() itself implements similar
 *    logic to the above, so be sure not to call it when you are
 *    "working behind libusb's back", as is the case here.
 * -# v_libusb_event_handler_active() determines if someone is currently
 *    holding the events lock
 *
 * You might be wondering why there is no function to wake up all threads
 * blocked on v_libusb_wait_for_event(). This is because libusb can do this
 * internally: it will wake up all such threads when someone calls
 * v_libusb_unlock_events() or when a transfer completes (at the point after its
 ...
 * -# v_libusb_open() will add another file descriptor to the poll set,
 *    therefore it is desirable to interrupt the event handler so that it
 *    restarts, picking up the new descriptor.
 * -# v_libusb_close() will remove a file descriptor from the poll set. There
 *    are all kinds of race conditions that could arise here, so it is
 *    important that nobody is doing event handling at this time.
 *
 * libusb handles these issues internally, so application developers do not
 * have to stop their event handlers while opening/closing devices. Here's how
 * it works, focusing on the v_libusb_close() situation first:
 *
 * -# During initialization, libusb opens an internal pipe, and it adds the read
 *    end of this pipe to the set of file descriptors to be polled.
 * -# During v_libusb_close(), libusb writes some dummy data on this event pipe.
 *    This immediately interrupts the event handler. libusb also records
 *    internally that it is trying to interrupt event handlers for this
 *    high-priority event.
 * -# At this point, some of the functions described above start behaving
 *    differently:
 *   - v_libusb_event_handling_ok() starts returning 1, indicating that it is NOT
 *     OK for event handling to continue.
 *   - v_libusb_try_lock_events() starts returning 1, indicating that another
 *     thread holds the event handling lock, even if the lock is uncontended.
 *   - v_libusb_event_handler_active() starts returning 1, indicating that
 ...
 *    v_libusb_close() operation a "free ride" to acquire the events lock. All
 *    threads that are competing to do event handling become event waiters.
 * -# With the events lock held inside v_libusb_close(), libusb can safely remove
 *    a file descriptor from the poll set, in the safety of knowledge that
 *    nobody is polling those descriptors or trying to access the poll set.
 * -# After obtaining the events lock, the close operation completes very
 *    quickly (usually a matter of milliseconds) and then immediately releases
 *    the events lock.
 * -# At the same time, the behaviour of v_libusb_event_handling_ok() and friends
 ...
 *
 * v_libusb_open() is similar, and is actually a more simplistic case. Upon a
 * call to v_libusb_open():
 *
 * -# The device is opened and a file descriptor is added to the poll set.
 * -# libusb sends some dummy data on the event pipe, and records that it
 *    is trying to modify the poll descriptor set.
 * -# The event handler is interrupted, and the same behaviour change as for
 *    v_libusb_close() takes effect, causing all event handling threads to become
 *    event waiters.
 * -# The v_libusb_open() implementation takes its free ride to the events lock.
 * -# Happy that it has successfully paused the events handler, v_libusb_open()
 ...
 * to v_libusb_event_handling_ok(): you must call this before every poll(), and
 * give up the events lock if instructed.
 */

int v_usbi_io_init(struct libusb_context *ctx)
{
	int r;
	...
	r = v_usbi_pipe(ctx->event_pipe);
	if (r < 0) {
		r = LIBUSB_ERROR_OTHER;
		goto err;
	}

	r = v_usbi_add_pollfd(ctx, ctx->event_pipe[0], POLLIN);
	if (r < 0)
		goto err_close_pipe;

#ifdef USBI_TIMERFD_AVAILABLE
	ctx->timerfd = timerfd_create(v_usbi_backend.get_timerfd_clockid(),
		TFD_NONBLOCK | TFD_CLOEXEC);
	if (ctx->timerfd >= 0) {
		usbi_dbg("using timerfd for timeouts");
		r = v_usbi_add_pollfd(ctx, ctx->timerfd, POLLIN);
		if (r < 0)
...
err_close_timerfd:
	close(ctx->timerfd);
	v_usbi_remove_pollfd(ctx, ctx->event_pipe[0]);
#endif
err_close_pipe:
...
void v_usbi_io_exit(struct libusb_context *ctx)
{
	v_usbi_remove_pollfd(ctx, ctx->event_pipe[0]);
	usbi_close(ctx->event_pipe[0]);
	usbi_close(ctx->event_pipe[1]);
	...
	if (usbi_using_timerfd(ctx)) {
		v_usbi_remove_pollfd(ctx, ctx->timerfd);
		close(ctx->timerfd);
	}
#endif
	usbi_mutex_destroy(&ctx->flying_transfers_lock);
...
static int calculate_timeout(struct usbi_transfer *transfer)
{
	...
	r = v_usbi_backend.clock_gettime(USBI_CLOCK_MONOTONIC, &current_time);
	if (r < 0) {
 ...
 */
DEFAULT_VISIBILITY
struct libusb_transfer * LIBUSB_CALL v_libusb_alloc_transfer(
	int iso_packets)
{
	struct libusb_transfer *transfer;
	size_t os_alloc_size = v_usbi_backend.transfer_priv_size;
 ...
 * allocated with v_libusb_alloc_transfer().
 *
 ...
 */
void API_EXPORTED v_libusb_free_transfer(struct libusb_transfer *transfer)
{
 ...
 */
int API_EXPORTED v_libusb_submit_transfer(struct libusb_transfer *transfer)
{
	struct usbi_transfer *itransfer =
	 ...
	 * the timeout handling and v_usbi_handle_disconnect paths.
	 *
	...
	r = v_usbi_backend.submit_transfer(itransfer);
	if (r == LIBUSB_SUCCESS) {
		itransfer->state_flags |= USBI_TRANSFER_IN_FLIGHT;
		/* keep a reference to this device */
		v_libusb_ref_device(transfer->dev_handle->dev);
	}
	usbi_mutex_unlock(&itransfer->lock);
 ...
 */
int API_EXPORTED v_libusb_cancel_transfer(struct libusb_transfer *transfer)
{
	struct usbi_transfer *itransfer =
		LIBUSB_TRANSFER_TO_USBI_TRANSFER(transfer);
	int r;
	...
	r = v_usbi_backend.cancel_transfer(itransfer);
	if (r < 0) {
 ...
 * \see v_libusb_alloc_streams()
 */
void API_EXPORTED v_libusb_transfer_set_stream_id(
 ...
 */
uint32_t API_EXPORTED v_libusb_transfer_get_stream_id(
	struct libusb_transfer *transfer)
{
	struct usbi_transfer *itransfer =
		LIBUSB_TRANSFER_TO_USBI_TRANSFER(transfer);
...
int v_usbi_handle_transfer_completion(struct usbi_transfer *itransfer,
	enum libusb_transfer_status status)
{
	...
	if (flags & LIBUSB_TRANSFER_FREE_TRANSFER)
		v_libusb_free_transfer(transfer);
	v_libusb_unref_device(dev_handle->dev);
	return r;
}

/* Similar to v_usbi_handle_transfer_completion() but exclusively for transfers
 * that were asynchronously cancelled. The same concerns w.r.t. freeing of
 * transfers exist here.
 * Do not call this function with the usbi_transfer lock held. User-specified
 * callback functions may attempt to directly resubmit the transfer, which
 * will attempt to take the lock. */
int v_usbi_handle_transfer_cancellation(struct usbi_transfer *transfer)
{
	struct libusb_context *ctx = ITRANSFER_CTX(transfer);
	uint8_t timed_out;
	...
	if (timed_out) {
		...
		return v_usbi_handle_transfer_completion(transfer, LIBUSB_TRANSFER_TIMED_OUT);
	}

	/* otherwise its a normal async cancel */
	return v_usbi_handle_transfer_completion(transfer, LIBUSB_TRANSFER_CANCELLED);
}
...
void v_usbi_signal_transfer_completion(struct usbi_transfer *transfer)
{
	struct libusb_context *ctx = ITRANSFER_CTX(transfer);
	int pending_events;
 ...
 * v_libusb_handle_events()) then you do not need to be concerned with this
 * locking.
 *
 * While holding this lock, you are trusted to actually be handling events.
 * If you are no longer handling events, you must call v_libusb_unlock_events()
 * as soon as possible.
 *
 ...
 * \ref libusb_mtasync
 */
int API_EXPORTED v_libusb_try_lock_events(libusb_context *ctx)
{
	int r;
	unsigned int ru;
 ...
 * \ref libusb_mtasync
 */
void API_EXPORTED v_libusb_lock_events(libusb_context *ctx)
{
	USBI_GET_CONTEXT(ctx);
...
/** \ingroup libusb_poll
 * Release the lock previously acquired with v_libusb_try_lock_events() or
 * v_libusb_lock_events(). Releasing this lock will wake up any threads blocked
 * on v_libusb_wait_for_event().
 *
 * \param ctx the context to operate on, or NULL for the default context
 * \ref libusb_mtasync
 */
void API_EXPORTED v_libusb_unlock_events(libusb_context *ctx)
{
	USBI_GET_CONTEXT(ctx);
 ...
 */
int API_EXPORTED v_libusb_event_handling_ok(libusb_context *ctx)
{
	unsigned int r;
	USBI_GET_CONTEXT(ctx);
 ...
 * \ref libusb_mtasync
 */
int API_EXPORTED v_libusb_event_handler_active(libusb_context *ctx)
{
	unsigned int r;
	USBI_GET_CONTEXT(ctx);
 ...
 * \ref libusb_mtasync
 */
void API_EXPORTED v_libusb_interrupt_event_handler(libusb_context *ctx)
{
	int pending_events;
	USBI_GET_CONTEXT(ctx);
 ...
 * some other thread is event handling so calling v_libusb_handle_events() is not
 * allowed.
 *
 * You then obtain this lock, re-check that another thread is still handling
 * events, then call v_libusb_wait_for_event().
 *
 * You only need to use this lock if you are developing an application
 * which calls poll() or select() on libusb's file descriptors directly,
 ...
 * \ref libusb_mtasync
 */
void API_EXPORTED v_libusb_lock_event_waiters(libusb_context *ctx)
{
	USBI_GET_CONTEXT(ctx);
	usbi_mutex_lock(&ctx->event_waiters_lock);
}
 ...
 * \ref libusb_mtasync
 */
void API_EXPORTED v_libusb_unlock_event_waiters(libusb_context *ctx)
{
	USBI_GET_CONTEXT(ctx);
	usbi_mutex_unlock(&ctx->event_waiters_lock);
}
 ...
 * with the event waiters lock held, see v_libusb_lock_event_waiters().
 *
 * This function will block until any of the following conditions are met:
 * -# The timeout expires
 * -# A transfer completes
 * -# A thread releases the event handling lock through v_libusb_unlock_events()
 *
 ...
 * \ref libusb_mtasync
 */
int API_EXPORTED v_libusb_wait_for_event(libusb_context *ctx, struct timeval *tv)
{
	int r;
	...
	r = v_usbi_cond_timedwait(&ctx->event_waiters_cond,
...
static void handle_timeout(struct usbi_transfer *itransfer)
{
	...
	r = v_libusb_cancel_transfer(transfer);
	if (r == LIBUSB_SUCCESS)
...
static int handle_timeouts_locked(struct libusb_context *ctx)
{
	...
	/* get current time */
	r = v_usbi_backend.clock_gettime(USBI_CLOCK_MONOTONIC, &systime_ts);
	if (r < 0)
		return r;
...
static int handle_events(struct libusb_context *ctx, struct timeval *tv)
{
	 ...
	 * v_libusb_handle_events() from within a hotplug or transfer callback) */
	...
	if (fds[0].revents) {
		...
		while (ret == 0 && !list_empty(&ctx->completed_transfers)) {
			...
			usbi_mutex_unlock(&ctx->event_data_lock);
			ret = v_usbi_backend.handle_transfer_completion(itransfer);
			if (ret)
		...
		while (!list_empty(&hotplug_msgs)) {
			...
			v_usbi_hotplug_match(ctx, message->device, message->event);

			/* the device left, dereference the device */
			if (LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT == message->event)
				v_libusb_unref_device(message->device);
	...
	r = v_usbi_backend.handle_events(ctx, fds + internal_nfds, nfds - internal_nfds, r);
	if (r)
	...
	struct timeval *out)
{
	...
	int r = v_libusb_get_next_timeout(ctx, &timeout);
	if (r) {
 ...
 * \ref libusb_mtasync
 */
int API_EXPORTED v_v_v_libusb_handle_events_timeout_completed(libusb_context *ctx,
...
retry:
	...
	r = v_libusb_wait_for_event(ctx, &poll_timeout);
 ...
 * Handle any pending events
 *
 * Like v_v_v_libusb_handle_events_timeout_completed(), but without the completed
 * parameter, calling this function is equivalent to calling
 * v_v_v_libusb_handle_events_timeout_completed() with a NULL completed parameter.
 *
 * This function is kept primarily for backwards compatibility.
 * All new code should call v_v_libusb_handle_events_completed() or
 * v_v_v_libusb_handle_events_timeout_completed() to avoid race conditions.
 *
 * \param ctx the context to operate on, or NULL for the default context
 * \param tv the maximum time to block waiting for events, or an all zero
 * timeval struct for non-blocking mode
 * \returns 0 on success, or a LIBUSB_ERROR code on failure
 */
int API_EXPORTED v_v_libusb_handle_events_timeout(libusb_context *ctx,
	struct timeval *tv)
{
	return v_v_v_libusb_handle_events_timeout_completed(ctx, tv, NULL);
}
 ...
 * for control over the timeout, use v_v_v_libusb_handle_events_timeout_completed()
 * instead.
 *
 * This function is kept primarily for backwards compatibility.
 * All new code should call v_v_libusb_handle_events_completed() or
 ...
 * \returns 0 on success, or a LIBUSB_ERROR code on failure
 */
int API_EXPORTED v_libusb_handle_events(libusb_context *ctx)
{
	struct timeval tv;
	tv.tv_sec = 60;
	tv.tv_usec = 0;
 ...
 *
 * Like v_libusb_handle_events(), with the addition of a completed parameter
 * to allow for race free waiting for the completion of a specific transfer.
 *
 * See v_v_v_libusb_handle_events_timeout_completed() for details on the completed
 * parameter.
 *
 * \param ctx the context to operate on, or NULL for the default context
 * \param completed pointer to completion integer to check, or NULL
 ...
 * \ref libusb_mtasync
 */
int API_EXPORTED v_v_libusb_handle_events_completed(libusb_context *ctx,
	int *completed)
{
	struct timeval tv;
	tv.tv_sec = 60;
	tv.tv_usec = 0;
	return v_v_v_libusb_handle_events_timeout_completed(ctx, &tv, completed);
}

/** \ingroup libusb_poll
 * Handle any pending events by polling file descriptors, without checking if
 * any other threads are already doing so. Must be called with the event lock
 * held, see v_libusb_lock_events().
 *
 * This function is designed to be called under the situation where you have
 * taken the event lock and are calling poll()/select() directly on libusb's
 * file descriptors (as opposed to using v_libusb_handle_events() or similar).
 ...
 * \ref libusb_mtasync
 */
int API_EXPORTED v_v_libusb_handle_events_locked(libusb_context *ctx,
	struct timeval *tv)
{
	int r;
	struct timeval poll_timeout;
 ...
 * descriptor set). The usual approach is to use v_libusb_get_next_timeout()
 ...
 * descriptor set, and v_libusb_get_next_timeout() will always return 0.
 ...
 * v_libusb_get_next_timeout(), or 1 if all timeout events are handled internally
 * or through regular activity on the file descriptors.
 * \ref libusb_pollmain "Polling libusb file descriptors for event handling"
 */
int API_EXPORTED v_libusb_pollfds_handle_timeouts(libusb_context *ctx)
{
 ...
 * are calling v_libusb_handle_events() or a variant directly.
 *
 ...
 *
 * When the timeout has expired, call into v_v_libusb_handle_events_timeout()
 * (perhaps in non-blocking mode) so that libusb can handle the timeout.
 *
 * This function may return 1 (success) and an all-zero timeval. If this is
 * the case, it indicates that libusb has a timeout that has already expired
 * so you should call v_v_libusb_handle_events_timeout() or similar immediately.
 ...
 */
int API_EXPORTED v_libusb_get_next_timeout(libusb_context *ctx,
	struct timeval *tv)
{
	struct usbi_transfer *transfer;
	...
	r = v_usbi_backend.clock_gettime(USBI_CLOCK_MONOTONIC, &cur_ts);
	if (r < 0) {
 ...
 * these notifiers (e.g. at v_libusb_init() time).
 *
 * Additionally, note that the removal notifier may be called during
 * v_libusb_exit() (e.g. when it is closing file descriptors that were opened
 * and added to the poll set at v_libusb_init() time). If you don't want this,
 * remove the notifiers immediately before calling v_libusb_exit().
 *
 * \param ctx the context to operate on, or NULL for the default context
 ...
 */
void API_EXPORTED v_libusb_set_pollfd_notifiers(libusb_context *ctx,
...
int v_usbi_add_pollfd(struct libusb_context *ctx, int fd, short events)
{
...
void v_usbi_remove_pollfd(struct libusb_context *ctx, int fd)
{
	struct usbi_pollfd *ipollfd;
	int found = 0;
 ...
 *
 * The returned list is NULL-terminated and should be freed with v_libusb_free_pollfds()
 ...
 */
DEFAULT_VISIBILITY
const struct libusb_pollfd ** LIBUSB_CALL v_libusb_get_pollfds(
	libusb_context *ctx)
{
#ifndef OS_WINDOWS
 ...
 * pollfd lists allocated with v_libusb_get_pollfds().
 *
 ...
 */
void API_EXPORTED v_libusb_free_pollfds(const struct libusb_pollfd **pollfds)
{
	if (!pollfds)
		return;
 ...
 */
void v_usbi_handle_disconnect(struct libusb_device_handle *dev_handle)
{
	struct usbi_transfer *cur;
	 ...
	 *    v_libusb_submit_transfer, has failed to submit and
	 *    v_libusb_submit_transfer is waiting for us to release the
	...
	while (1) {
		...
		v_usbi_backend.clear_transfer_priv(to_cancel);
		usbi_mutex_unlock(&to_cancel->lock);
		v_usbi_handle_transfer_completion(to_cancel, LIBUSB_TRANSFER_NO_DEVICE);
	}
