...
func enumerateRaw(vendorID uint16, productID uint16) ([]DeviceInfo, error) {
	...
	for _, info := range infos {
		C.v_libusb_unref_device(info.rawDevice.(*C.libusb_device))
	}
...
func enumerateRawWithRef(vendorID uint16, productID uint16) ([]DeviceInfo, error) {
	...
	if C.ctx == nil {
		if err := fromRawErrno(C.v_libusb_init((**C.libusb_context)(&C.ctx))); err != nil {
	...
	count := C.v_v_libusb_get_device_list(C.ctx, &deviceList)
	if count < 0 {
		return nil, rawError(count)
	}
	defer C.v_libusb_free_device_list(deviceList, 1)
	...
	*(*reflect.SliceHeader)(unsafe.Pointer(&devices)) = reflect.SliceHeader{
	...
	for devnum, dev := range devices {
		...
		if err := fromRawErrno(C.v_v_libusb_get_device_descriptor(dev, &desc)); err != nil {
		...
		for cfgnum := 0; cfgnum < int(desc.bNumConfigurations); cfgnum++ {
			...
			if err := fromRawErrno(C.v_libusb_get_config_descriptor(dev, C.uint8_t(cfgnum), &cfg)); err != nil {
				...
				for _, alt := range alts {
					...
					*(*reflect.SliceHeader)(unsafe.Pointer(&ends)) = reflect.SliceHeader{
					...
					if reader != nil && writer != nil {
						...
						C.v_libusb_ref_device(dev)

						port := uint8(C.v_libusb_get_port_number(dev))
...
func openRaw(info DeviceInfo) (*rawDevice, error) {
	...
	if err != nil {
		...
		for _, match := range matches {
			C.v_libusb_unref_device(match.rawDevice.(*C.libusb_device))
		}
		return nil, err
	}
	...
	if err := fromRawErrno(C.v_libusb_open(info.rawDevice.(*C.libusb_device), (**C.struct_libusb_device_handle)(&handle))); err != nil {
		return nil, fmt.Errorf("failed to open device: %v", err)
	}
	if err := fromRawErrno(C.v_libusb_claim_interface(handle, (C.int)(info.Interface))); err != nil {
		C.v_libusb_close(handle)
...
func (dev *rawDevice) Close() error {
	...
	if dev.handle != nil {
		C.v_libusb_release_interface(dev.handle, (C.int)(dev.Interface))
		C.v_libusb_close(dev.handle)
		dev.handle = nil
	}
	C.v_libusb_unref_device(dev.rawDevice.(*C.libusb_device))
...
func (dev *rawDevice) Write(b []byte) (int, error) {
	...
	if err := fromRawErrno(C.v_libusb_interrupt_transfer(dev.handle, (C.uchar)(*dev.rawWriter), (*C.uchar)(&b[0]), (C.int)(len(b)), &transferred, (C.uint)(0))); err != nil {
...
func (dev *rawDevice) Read(b []byte) (int, error) {
	...
	var transferred C.int
	if err := fromRawErrno(C.v_libusb_interrupt_transfer(dev.handle, (C.uchar)(*dev.rawReader), (*C.uchar)(&b[0]), (C.int)(len(b)), &transferred, (C.uint)(0))); err != nil {
